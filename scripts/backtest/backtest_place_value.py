"""
複勝バリューベット戦略（予測確率 vs 市場確率）
- 予測1位に盲目的に賭けるのではなく
- 予測確率がオッズから計算される市場確率を上回る馬に賭ける
- 単勝ではなく複勝（1-3着）で判定
"""
...
def estimate_place_odds(win_odds, num_horses):
    """
    単勝オッズから複勝オッズを推定
...
# 特徴量抽出関数（backtest_hybrid_modelと同じ）
...
for idx, race_id in enumerate(odds_race_ids):
    ...
    for i, horse_data in enumerate(horses_data):
        ...
        horse_data['market_prob'] = 1.0 / horse_data['odds']  # オッズから市場確率
    ...
    for threshold in value_thresholds:
        ...
        if best_horse['actual_rank'] <= 3:
            ...
            results_by_threshold[threshold]['total_return'] += place_odds * 100

print("\n" + "="*80)
print("複勝バリューベット戦略 結果")
print("="*80)
...
for threshold in value_thresholds:
    ...
    print("{:>10.0f}% | {:>8d} | {:>8d} | {:>12,d}円 | {:>12,.0f}円 | {:>7.1f}%".format(
        threshold * 100, total_bets, total_wins, total_investment, total_return, recovery_rate
    ))
