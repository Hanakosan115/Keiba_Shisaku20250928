"""
2024年データでの実戦シミュレーション
完全なレース結果で予測モデルの精度と収益性をテスト
"""
...
# データ分割
# 訓練データ: 2020-2023年
df_train = df[(df['date'] >= '2020-01-01') & (df['date'] < '2024-01-01')].copy()

# テストデータ: 2024年（完全なレース結果がある行のみ）
df_test_all = df[(df['date'] >= '2024-01-01') & (df['date'] < '2025-01-01')].copy()
...
if len(df_test) == 0:
    print("⚠️ 2024年のデータが見つかりません")
    exit(1)

if rank_1_count == 0:
    print("⚠️ 2024年データに1着馬が見つかりません")
    print("データ構造を確認してください")
    exit(1)
