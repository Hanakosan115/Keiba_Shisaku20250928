...
print("="*80)
print("オッズなしLightGBMモデル訓練")
print("  - オッズを特徴量から除外")
print("  - 人気を特徴量から除外")
print("  - 市場評価に依存しない独自予測")
print("="*80)
...
for idx, race_id in enumerate(train_races):
    ...
    for _, horse in race_horses.iterrows():
        ...
        # ★ 特徴量ベクトル（45次元）★
        # オッズ (np.log1p(odds)) と 人気 (ninki) を除外
        feature_vector = [
            # 基本特徴量 (20次元) - オッズと人気を除外
...
print(f"\n訓練サンプル数: {len(X_all):,}件")
print(f"特徴量次元: {X_all.shape[1]}次元 (オッズ・人気を除外)")
...
# モデル保存
model_file = 'lgbm_model_no_odds.pkl'
