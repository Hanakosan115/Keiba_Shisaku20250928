...
# データ読み込み
df = pd.read_csv(r"C:\Users\bu158\HorseRacingAnalyzer\data\netkeiba_data_combined_202001_202510.csv", encoding='utf-8', low_memory=False)
...
for idx, race_id in enumerate(race_ids[:100]):  # 最初の100レースで検証
    ...
    # ランダムに3頭選択
    all_umabans = race_horses['Umaban'].tolist()
    if len(all_umabans) < 3:
        continue

    random_3_horses = random.sample(all_umabans, 3)
    ...
    try:
        ...
        # ランダム3頭の馬連BOXを生成
        umaren_pairs = list(combinations(random_3_horses, 2))
...
print("・AI予測（Ninki未使用）: 99.9%")
print(f"・ランダム予測: {umaren_hits/umaren_total*100:.1f}%")
print("=" * 80)
