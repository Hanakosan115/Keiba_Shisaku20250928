...
print(f"2024年データ: {len(test_df):,}件")

# 最初の100レースだけテスト
test_races = test_df['race_id'].unique()[:100]
print(f"テストレース数: {len(test_races)}レース（サンプル）")
...
# バリュー判定の閾値
value_thresholds = [0.0, 0.10, 0.20, 0.30]
...
for idx, race_id in enumerate(test_races):
    if (idx + 1) % 20 == 0:
        print(f"  進捗: {idx+1}/{len(test_races)}")
