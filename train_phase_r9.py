"""
Phase R9 モデル訓練
78特徴量（R8+Optuna）+ 1個の新特徴量 = 79特徴量

追加特徴量:
  training_rank_num : 調教評価数値（A=4, B=3, C=2, D=1, 不明=2）

使い方:
  py train_phase_r9.py                  # R8+Optunaパラメータ流用
  py train_phase_r9.py --optuna 100     # Optuna最適化
"""
...
parser.add_argument('--log', type=str, default='train_phase_r9.log')
...
print("=" * 80)
print("  Phase R9 モデル訓練（78 + 1 = 79特徴量）")
print("=" * 80)
...
FEATURE_LIST = BASE_FEATURES + R9_FEATURES
print(f"特徴量数: {len(BASE_FEATURES)}(R8) + {len(R9_FEATURES)}(R9) = {len(FEATURE_LIST)}")

# データ
print("データ読み込み中...")
df_train = pd.read_csv('data/phase_r9/train_features.csv', low_memory=False)
df_val   = pd.read_csv('data/phase_r9/val_features.csv',   low_memory=False)
df_test  = pd.read_csv('data/phase_r9/test_features.csv',  low_memory=False)
...
if args.optuna > 0:
    ...
    best_params = {'objective': 'binary', 'metric': 'binary_logloss',
                   ...
                   **best.params}
    os.makedirs('models/phase_r9', exist_ok=True)
    with open('models/phase_r9/optuna_study.pkl', 'wb') as f:
        pickle.dump(study, f)
...
for i, (feat, val) in enumerate(imp.head(25).items()):
    r9_mark = ' ★R9新規' if feat in R9_FEATURES else ''
    tee(f"  {feat:<45} {val:>10.0f}{r9_mark}")

tee("\n【R9特徴量ランキング】")
for feat in R9_FEATURES:
    rank_pos = list(imp.index).index(feat) + 1
    tee(f"  {rank_pos:3d}位 {feat:<45} gain={imp[feat]:>8.0f}")

# 比較
tee("\n【R8+Optuna vs R9 比較】")
tee(f"  {'指標':<20} {'R8+Optuna':>12} {'R9':>12}")
...
# 保存
r9_dir = 'models/phase_r9'
os.makedirs(r9_dir, exist_ok=True)
win_model.save_model(f'{r9_dir}/model_win.txt')
place_model.save_model(f'{r9_dir}/model_place.txt')
with open(f'{r9_dir}/feature_list.pkl', 'wb') as f:
...
with open(f'{r9_dir}/metadata.json', 'w', encoding='utf-8') as f:
...
tee("=" * 80)
tee(f"\n保存先: {r9_dir}/")
tee(f"\n【次のステップ — R9をアクティブにしてバックテスト】")
tee(f"  copy models\\phase_r9\\model_win.txt     phase14_model_win.txt")
tee(f"  copy models\\phase_r9\\model_place.txt   phase14_model_place.txt")
tee(f"  copy models\\phase_r9\\feature_list.pkl  phase14_feature_list.pkl")
tee(f"  py run_gui_backtest.py --year 2024")
tee(f"  py run_gui_backtest.py --year 2025")
tee(f"\n【R9が振るわない場合のロールバック】")
tee(f"  py restore_model.py phase_r8")
