"""
Phase R5 モデル訓練
67特徴量（R4）+ 7個のペース・脚質特徴量 = 74特徴量

追加特徴量:
  avg_first_corner_fixed  : 1コーナー通過順平均（Passageから正しく計算）
  avg_last_corner_fixed   : 最終コーナー通過順平均
  avg_position_change_v2  : コーナー間の位置変化量
  running_style_v2        : 修正脚質（77%差しバグ修正）
  slightly_heavy_win_rate : 稍重馬場勝率
  field_escape_count      : 出走馬中の逃げ・先行馬数
  field_pace_advantage    : この馬の脚質が展開上有利か

使い方:
  py train_phase_r5.py                    # R4パラメータ流用（高速）
  py train_phase_r5.py --optuna 40        # Optunaで最適化
"""
...
parser.add_argument('--optuna', type=int, default=0, help='Optunaトライアル数（0=スキップ）')
parser.add_argument('--log', type=str, default='train_phase_r5.log', help='ログファイルパス')
...
print("=" * 80)
print("  Phase R5 モデル訓練（67 + 7 = 74特徴量）")
print("=" * 80)
...
FEATURE_LIST = BASE_FEATURES + R5_FEATURES
print(f"特徴量数: {len(BASE_FEATURES)}(R4) + {len(R5_FEATURES)}(R5) = {len(FEATURE_LIST)}")

# ===================================================================
# データ読み込み
# ===================================================================
print("\nデータ読み込み中...")
df_train = pd.read_csv('data/phase_r5/train_features.csv', low_memory=False)
df_val   = pd.read_csv('data/phase_r5/val_features.csv',   low_memory=False)
df_test  = pd.read_csv('data/phase_r5/test_features.csv',  low_memory=False)
...
if args.optuna > 0:
    ...
    # study を保存（途中結果の保全）
    with open('models/phase_r5/optuna_study.pkl', 'wb') as _f:
...
for _, row in importance.head(20).iterrows():
    tag = ' ★R5新規' if row['feature'] in R5_FEATURES else ''
    print(f"  {row['feature']:<40s} {row['importance']:>8.0f}{tag}")

# ===================================================================
# モデル保存
# ===================================================================
print("\nモデル保存中...")
r5_dir = 'models/phase_r5'
os.makedirs(r5_dir, exist_ok=True)

model_win.save_model(  f'{r5_dir}/model_win.txt')
model_place.save_model(f'{r5_dir}/model_place.txt')
with open(f'{r5_dir}/feature_list.pkl', 'wb') as f:
...
with open(f'{r5_dir}/metadata.json', 'w', encoding='utf-8') as f:
...
print()
print("【次のステップ】")
print("  # R5 モデルをアクティブにしてバックテスト:")
print("  copy models\\phase_r5\\model_win.txt     phase14_model_win.txt")
print("  copy models\\phase_r5\\model_place.txt   phase14_model_place.txt")
print("  copy models\\phase_r5\\feature_list.pkl  phase14_feature_list.pkl")
